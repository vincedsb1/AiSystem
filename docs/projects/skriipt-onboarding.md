# Skriipt - AI System onboarding

## Politique d'installation

Skriipt installe localement les skills Codex `shared.*` declares dans
`projects[].install_shared_skills` dans `skills-registry.yml`.

Les exports sont generes dans `Skriipt/.agents/skills/<skill>/SKILL.md` depuis
...
Le socle installe est identique a InterviewOS, intrai, Pylaa et Pylot:
...
Skriipt ne recoit aucun canonical `aimoto.*`, `interviewos.*`, `intrai.*`,
`pylaa.*` ou `pylot.*`. Aucun canonical `skriipt.*` n'est cree tant qu'un
besoin projet explicite ne le justifie pas.
...
canonical `skriipt.*` explicite et retirer l'exception correspondante.
...
grep -R "aimoto\|AIMOTO\|InterviewOS\|intrai\|Pylaa\|Pylot\|BTC\|backtesting\|forecast\|strategy-loop\|AIMOTO_DATA_DIR" \
  .agents/skills || true
```
...
- six `expected_claude_only` pour les commandes Skriipt listees;
