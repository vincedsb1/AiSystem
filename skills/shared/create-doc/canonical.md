...
1. Document type (`PAGE` / `COMPONENT` / `HOOK` / `MODULE` / `GUIDE` / `API` / `STORAGE`, or whatever types are defined in `project-config.md`).
2. Name (kebab-case, e.g. `runs-registry`).
...
**Do not use for** : phase specs (those have their own `/spec-*` workflow), audits (manual edits in `/docs/audits/`), research notes.
...
After auto-detection, **display the detected configuration once** and confirm before writing.
...
```
Read mandatorily : <doc_rules>   (from project-config.md or auto-detected)
```
...
If no such file exists, infer the rules from existing docs of the same type (read at least 2 existing examples).

---

## PHASE 2 — Decide : new file vs enrichment

Before creating a file, verify (typical 3-criteria rule) :
...
If fewer than 2 criteria → **enrich an existing file**, do not create a new one.
...
Filename pattern : `<PREFIX><KEBAB-UPPER>.md` (e.g. `PAGE-RUNS-REGISTRY.md`).
...
**Category**: <Backend Module | Frontend Page | Frontend Component | Frontend Hook | Storage | API | Guide>
...
### Type-specific sections (generic defaults — override from `project-config.md` if defined)
...
- **Every code example** must have :
  ...
  - Typed fenced block (` ```python `, ` ```ts `, …).
  - **Explanation** of the why/when.
- **No redundancy** : if an endpoint already exists in `API-ENDPOINTS.md`, **link to it** ; do not duplicate.
- **No paraphrase** : copy real source code, do not invent.
- **Target length** : 80–300 lines. > 500 = split.
- **Markdown tables** for any list of parameters, props, states.
- **No decorative emojis** (status markers `✅` / `🚧` / `❌` are allowed).
- **No time estimates.**
- **No mention of AI / Claude / Copilot / automation.**
...
- Mention Claude / Copilot / AI / automation in the content.
- Use decorative emojis (status `✅` / `🚧` / `❌` allowed).
